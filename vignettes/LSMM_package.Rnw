...
\author{Jingsi Ming$~^1$, Mingwei Dai$~^{1,2}$, Mingxuan Cai$~^1$, Jin Liu$~^3$, and Can Yang$~^4$\\
...
an efficient statistical approach to integrating functional annotations with genome-wide association studies. It provides model parameter estimation and statistical inference for risk SNPs and relevant annotations.
...
<<LSMM-prelim, warning=FALSE>>=
library("LSMM")
@

This vignette is organized as follows.
Section \ref{fitting} discusses how to fit LSMM in various settings.
Section \ref{association} explains command lines for statistical inference of risk SNPs and relevant annotations using LSMM.

Always feel free to contact Jingsi Ming at \texttt{jsming@life.hkbu.edu.hk} for any questions or suggestions regarding the `\texttt{LSMM}' package.

\section{Workflow}\label{workflow}

In this vignette, we use the simulated ExampleData in the package. We set the number of SNPs, fixed effects and random effects to be 100,000, 10 and 500 respectively. Users can find the $p$-value in the `\texttt{ExampleData\$Pvalue}', fixed effects in `\texttt{ExampleData\$Z}' and random effects in `\texttt{ExampleData\$A}'.
...
dim(A)
@
The length of `\texttt{Pvalue}' is assumed to be the same as the number of rows of matrix provided to `\texttt{Z}' and `\texttt{A}'. When we analyze real data, we need to take the intersection of the SNPs in `\texttt{Pvalue}', `\texttt{Z}' and `\texttt{A}' and keep the SNPs in the same order.
...
When we also have related annotation data, this annotation data can be easily incorporated into LSMM by providing it in the second or the third argument of `\texttt{LSMM}' function. The second argument is regarded as fixed effect and the third argument is regarded as random effect.
...
\subsection{Statistical inference for risk SNPs and relevant annotations}\label{association}

Now, based on the fitted LSMM, we can make statistical inference for risk SNPs:
<<LSMM-assoc.SNP>>=
...
We can also make statistical inference for relevant annotations:
<<LSMM-relev.Anno>>=
...
\bibitem{LSMM} Jingsi Ming, Mingwei Dai, Mingxuan Cai, Jin Liu, Can Yang. LSMM: A statistical approach to integrating functional annotations with genome-wide association studies. 2017. Under review.
